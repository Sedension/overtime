...
public partial class leader_personnel : System.Web.UI.Page
{
    ...
    public void databind()
    {
        ...
        SqlCommand cmd = new SqlCommand("select * from all_personnel where department='" + department + "'", conn);//访问数据库的SQL语句存到了cmd中
    ...
    public void databind1()
    {
        ...
        conn.ConnectionString = ConfigurationManager.AppSettings["ConnectionString"];
        string department = Session["department"].ToString();
        SqlCommand cmd = new SqlCommand("select * from all_personnel where " + DropDownList1.Text + " like '%" + TextBox1.Text.Trim() + "%' and department='" + department + "'", conn);//访问数据库的SQL语句存到了cmd中
        DataTable dt1 = new DataTable();
        SqlDataAdapter adp = new SqlDataAdapter(cmd);//数据适配器 执行cmd
        adp.Fill(dt1);
